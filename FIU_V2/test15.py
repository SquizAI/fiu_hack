...
def chat(qa_chain, userQuestion):
    result = qa_chain({"question": userQuestion})
    answer = result.get('answer', "").strip()
    if not answer or "I don't know." in answer:
        return {"answer": "I don't seem to know this. Please reach out to helpdesk at hd@company.com"}
...
def main():
    ...
    page = st.sidebar.radio("Go to", ["Home", "Crime Analysis", "Social Media Analysis", "Traffic Analysis", "Real Estate Trends", "Weather Forecast", "News and Search Results", "Chat with AI"])
    ...
    if page == "Home":
        ...
        st.subheader("Chat with AI about Coral Gables")
        
        qa_chain = initialize_qa_chain()
        
        user_question = st.text_input("Ask a question about Coral Gables:")
        if st.button("Ask"):
            if not os.environ.get("OPENAI_API_KEY"):
                st.error("Please set your OpenAI API key.")
            else:
                with st.spinner("Thinking..."):
                    response = chat(qa_chain, user_question)
